---
title: "SV calling from long reads in individual `r params$sample`"
date: "`r Sys.Date()`"
...
if (!is.null(params$sample) && nzchar(params$sample)) sample <- params$sample

# knitr sets the working directory to this file's location, hence the ../../ prefix.
# The reference files are shared by all individuals; everything else is per individual.
path_shared <- function(...) paste0("../../", wdir, "/", ...)
path_in <- function(...) paste0("../../", wdir, "/", sample, "/", ...)

assembly_path  <- path_shared("genome/", genome, "_assembly_data_report.jsonl")
seqreport_path <- path_shared("genome/", genome, "_sequence_report.jsonl")
