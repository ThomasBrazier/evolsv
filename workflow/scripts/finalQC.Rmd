...
```{r mapping, echo = F}
mapping = readLines(paste0(wdir, "/", sra, "/mapping/", sa, "_mapping.stats.tsv"))

print(gsub("\t", " ", mapping))
```


The diagnostic plots for mapping are in the `mapping/` directory.
...
```{r loadingSV, echo = F}
sniffles = read.vcfR(paste0(wdir, "/", sra, "/", sra, "_sniffles_noBND.vcf"))
...
```{r loadingSV, echo = F}
merged = read.vcfR(paste0(wdir, "/", sra, "/", sra, "_merged_noBND.vcf"))
...
geno = read.vcfR(paste0(wdir, "/", sra, "/", sra, "_merged_genotype.vcf"))
...
```{r PL, echo = T}
df = data.frame(GT = c(rep("0/0", sum(gt.geno$GT %in% c("0/1", "1/0"))),
                       rep("0/1", sum(gt.geno$GT %in% c("0/1", "1/0"))),
                       rep("1/0", sum(gt.geno$GT %in% c("0/1", "1/0")))),
                PL = c(gt.geno$PL_HOMREF[which(gt.geno$GT %in% c("0/1", "1/0"))],
                       gt.geno$PL_HET[which(gt.geno$GT %in% c("0/1", "1/0"))],
                       gt.geno$PL_HOMALT[which(gt.geno$GT %in% c("0/1", "1/0"))]))
