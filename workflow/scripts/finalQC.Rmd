...
The cumulative number of calls (pooling tools) as a function of the number of supporting tools. If almost all SVs have been called (in a perfect world), we would expect an asymptotic curve towards a plateau when all individual tools are pooled together.
...
for (type in c("DEL", "INS", "DUP", "INV")) {
  ...
  for (i in 1:nrow(performance_per_type)) {
    performance_per_type$true_positive[i] = sum(genotype_subset[,performance_per_type$tool[i]] == "HET") / n_calls
    performance_per_type$false_positive[i] = sum(genotype_subset[,performance_per_type$tool[i]] == "HOM") / n_calls
    het_in_one_tool = apply(genotype_subset, 1, function(x) {"HET" %in% x})
    performance_per_type$false_negative[i] = sum(genotype_subset[,performance_per_type$tool[i]] == "MISSING" & het_in_one_tool) / n_calls
  }
