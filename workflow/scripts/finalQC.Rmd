...
fix.merged$svim = ifelse(lapply(strsplit(fix.merged$supp_vec, ""), `[[`, 1) == "1" | lapply(strsplit(fix.merged$supp_vec, ""), `[[`, 5) == "1", 1, 0)
fix.merged$sniffles = ifelse(lapply(strsplit(fix.merged$supp_vec, ""), `[[`, 2) == "1" | lapply(strsplit(fix.merged$supp_vec, ""), `[[`, 6) == "1", 1, 0)
...
```{r SVqualityAligner, echo = F, warning = F}
...
```{r SVqualityCaller, echo = F, warning = F}
...
p1 = ggplot(fix.merged, aes(y = abs(sv_length), x = sv_type, fill = sv_type)) +
  geom_boxplot() +
  ylab("SV Length (bp)") + xlab("SV type") +
  labs(fill = "SV type", colour = "SV type") +
  theme_bw()
...
for (i in 1:nrow(performance)) {
  performance$true_positive[i] = sum(genotype[,performance$tool[i]] == "HET") / n_calls
  performance$false_positive[i] = sum(genotype[,performance$tool[i]] == "HOM") / n_calls
  het_in_one_tool = apply(genotype, 1, function(x) {"HET" %in% x})
  performance$false_negative[i] = sum(genotype[,performance$tool[i]] == "MISSING" & het_in_one_tool) / n_calls
}
...
```{r, echo = T, tab.cap = "Performance of the different SV calling tools"}
...
The cumulative number of calls (pooling tools) as a function of the number of supporting tools. If almost all SVs have been called , we expect an asymptotic curve towards a plateau when all individual tools are pooled together.
...
cumsum(table(fix.merged$n_supp)[8:1])

df = data.frame(num_tools = 1:8,
                cumsum_num_sv = cumsum(table(fix.merged$n_supp)[8:1]))
```




```{r, fig.height = 24, echo = F, warning = F}
ggplot(df, aes(x = num_tools, y = cumsum_num_sv)) +
  geom_point() +
  geom_line() +
  xlab("Number of supporting tools") + ylab("Cumulative number of calls (tools pooled)") +
  theme_bw()
```
...
An alternative measure of confidence of each call.
